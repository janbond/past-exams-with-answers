...
data/yingwenzidanci.json  「讀書」分頁－英文單字整理（非考古題，是從法學知識與英文題庫萃取的單字複習卡）
```
...
新增：頂層新增「讀書」分頁，用來放「非考古題原題」的整理型讀書資料（例如單字表、重點整理），與其他科目分頁（都是逐題的考古題）區隔開。「讀書」分頁底下用 track 分「書」（目前有「英文單字」「台灣政策雷達」兩本），每本書再用 section 分「章節」。之後若要加其他科目的重點整理，可以比照這個模式：subject固定用"讀書"，track換成新的書名，section換成章節名稱即可，不用改app.js。

新增：`data/zhengce_leida.json` 是「讀書」分頁底下的「台灣政策雷達」——彙整自考生自行提供的政策動態文件，依 AI 治理、資通安全、打詐監理、個資治理、數位政府五大領域分成 5 章，每章包含「重點知識彙整」＋2 題模擬申論題與四段式（定義→為何需要→核心要素→挑戰）詳解。所有模擬題皆標示「〔模擬預測題〕」，非真實考古題。

新增：「讀書」分頁改為「章節式」瀏覽——選定一本書（track）後，畫面會出現「章節目錄」，每次只顯示一章的內容，不用再滑動整頁尋找；並且會自動記錄「上次讀到哪一章」（存在瀏覽器 localStorage 的 `bookprogress_v1`），下次重新整理／回訪同一本書時會自動跳回上次的章節，章節目錄中已讀過的章節也會標示「✓ 已讀」。每章底部有「上一章／下一章」按鈕可連續閱讀。此機制只套用在 subject="讀書" 的內容，不影響其他考古題科目原本的年份／類別瀏覽方式。
